<template>
  <div class="bg-white p-12 rounded-lg flex flex-col items-start justify-between gap-12 min-w-[160px] min-h-[160px]" :class="{'cursor-pointer hover:scale-[0.99] hover:bg-stone-50 duration-75': !!description, [icon]: icon}"  @click="open = !open && !!description" v-bind="$attrs">
    <FontAwesomeIcon
      v-if="icon"
      :icon="['fas', icon]"
