<template>
  <div class="bg-white p-12 rounded-2xl flex flex-col items-start justify-between gap-12 min-w-[160px] min-h-[160px]" :class="{'cursor-pointer hover:scale-[0.99] hover:bg-stone-50 duration-75': !!description, [icon]: icon}"  @click="open = !open && !!description" v-bind="$attrs">
    <FontAwesomeIcon
      v-if="icon"
      :icon="['fas', icon]"
  ...
  <Transition name="page" style="--total: 1; --i: 1">
    <div v-if="open" class="fixed flex items-center justify-center top-0 left-0 w-screen h-screen bg-black/60 z-50" @click.self="open = !open">
      <div class="bg-white rounded-2xl p-8 m-6 sm:p-12 shadow-2xl max-w-2xl max-h-[90vh] overflow-auto">
